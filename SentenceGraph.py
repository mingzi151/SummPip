...
def get_nouns_for_verbs(string):
    doc = spacynlp(string)
    nouns_list = []
    if len(doc)>0:
        for token in doc:
            ...
            if token.pos_ == "VERB" and token.text not in verbs_to_escape:
                ...
                noun_forms =_nounify(token.text)
...
def find_most_similar_words(word_vectors,nouns_list,threshold=0.65):
    similar_nouns_list=[]
...
def check_noun_reference(similar_nouns_list, subsequent_sen):
    flag=False
...
def check_discourse_markers(str1,str2):
    flag = False
    doc2 = spacynlp(str2)
    if len(doc2)>0:
...
def make_graph_undirected(source, target, weight):
...
class SentenceGraph:
    def __init__(self, sentences_list, w2v, lm_model, lm_tokenizer, cluster_algo="spc",threshold=0.65,ita=0.9):
    ...
    # get sentence embeddings with gpt2
    def get_lm_embedding(self, string):
        sent=string.lower()
        if len(sent)!= 0:
            input_ids = torch.tensor([self.lm_tokenizer.encode(sent)])
            ...
            for i in range(self.length):
                ...
                for j in range(i+1,self.length):
                    ...
                    if (j-i) == 1:
                        ...
                        nouns_list = get_nouns_for_verbs(sen_i)
                        # get most similar words for above nouns, including itself
                        similar_nouns_list = find_most_similar_words(glove_word_vectors, nouns_list,self.threshold)
                        # check for devebal noun 
                        flag = check_noun_reference(similar_nouns_list, sen_j)
                        if not flag:
                            # check for disourse markers
                            flag=check_discourse_markers(sen_i,sen_j)
                    else:
                        # check for name entities
                        flag=compare_name_entity(sen_i,sen_j)
